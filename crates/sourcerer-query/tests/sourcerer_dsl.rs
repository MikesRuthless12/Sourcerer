//! Phase 10 Sourcerer-DSL fixture.
//!
//! 200+ queries spanning every Sourcerer-only surface: the Phase-9
...
//!
//! The Phase-10 prompt explicitly gates 200+ Sourcerer-DSL queries
...
fn strict_everything_rejects_audio_modifiers() {
    use sourcerer_query::ParseError;
    for q in [
        "lufs:<-14",
        "codec:flac",
        "length:>3:00",
...
fn strict_everything_rejects_similar_modifier() {
    ...
    assert!(matches!(
        err,
        sourcerer_query::ParseError::StrictEverythingViolation { .. }
    ));
}
...
#[test]
fn two_hundred_sourcerer_dsl_queries_parse() {
    let mut all: Vec<String> = hand_curated();
    all.extend(generated());
    assert!(
        all.len() >= 200,
        "Phase 10 gates 200+ Sourcerer-DSL queries; got {}",
        all.len()
    );
    for q in &all {
