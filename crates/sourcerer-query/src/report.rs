...
/// 4 GiB are not a use case Sourcerer ships toward).
...
pub fn parse_to_report(s: &str, opts: ParseOpts) -> ParseReport {
    ...
    // Strict-everything: collect every Sourcerer-only modifier /
    ...
    if opts.strict_everything {
        for ti in tokens.iter() {
            match &ti.kind {
                TokenKind::Modifier { name } if is_sourcerer_only_modifier(name) => {
                    errors.push(ErrorInfo {
                        span: ti.span,
                        message: format!(
...
fn is_sourcerer_only_modifier(key: &str) -> bool {
    matches!(
...
mod tests {
    ...
    fn strict_everything_collects_multiple_violations() {
        // Three Sourcerer-only modifiers in one query — strict mode
