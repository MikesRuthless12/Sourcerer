...
fn main() {
    ...
    let cursor_root = canonical_scratch
        ...
        .unwrap_or_else(|| std::env::temp_dir().join("freally-phase03-cursors"));
    ...
    for i in 0..args.creates {
        ...
        std::fs::write(&p, b"freally phase 3 smoke").expect("write create");
    ...
    for p in created_paths.iter().take(args.modifies) {
        std::fs::write(p, b"freally phase 3 smoke - modified").expect("write modify");
    }
