...
/// existing directory. Cursor lives under `~/.local/share/freally/cursors/`.
...
impl JournalSubscriber {
    ...
    pub fn bootstrap(&self) -> impl Stream<Item = JournalEvent> + Send + 'static {
        ...
        std::thread::Builder::new()
            .name("freally-journal-lin/bootstrap".into())
            .spawn(move || {
    ...
    pub fn subscribe(&self) -> impl Stream<Item = JournalEvent> + Send + 'static {
        ...
        std::thread::Builder::new()
            .name("freally-journal-lin/subscribe".into())
            .spawn(move || {
            ...
            match kind {
                ...
                ReasonKind::Create => {
                    ...
                    if flags::is_dir_inotify(ev.mask) {
                        if let Ok(wd) = inotify.add_watch(&path, FREALLY_INOTIFY_MASK) {
...
fn add_watch_recursive(
    ...
    match inotify.add_watch(root, FREALLY_INOTIFY_MASK) {
        Ok(wd) => {
    ...
    for sub in collect_subdirs(root) {
        match inotify.add_watch(&sub, FREALLY_INOTIFY_MASK) {
            Ok(wd) => {
...
fn subscribe_fanotify(
    ...
    fanotify
        .mark_filesystem(root, FREALLY_FANOTIFY_MASK)
