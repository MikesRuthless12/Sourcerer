//! Freally filename similarity lens (Phase 6).
//!
//! Exposes a [`SimilarityIndex`] that consumes the same `JournalEvent`
//! stream as `freally-index` and answers near-duplicate filename
...
/// `freally-index`'s `derive_file_id` so the two indexes stay keyed by
/// the same value. We can't depend on `freally-index` here without a
/// cycle, so we re-implement the same blake3-truncated hash inline.
fn file_id_and_name(path: &std::path::Path) -> (u64, String) {
    let bytes = path.as_os_str().as_encoded_bytes();
    // Same construction as `freally_index::derive_file_id`; the two
...
/// `freally_index::derive_file_id`. The two crates need to agree on
...
fn blake3_truncated(bytes: &[u8]) -> u64 {
    ...
    // Freally-index stores the `file_id` as `i64` (SQLite INTEGER)
