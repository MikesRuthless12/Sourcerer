...
//!   3. The macOS app menu (Sourcerer → Preferences / Quit) is wired
//!      to the same command ids the in-window menu uses, so a click
//!      from either path runs the same handler.

use sourcerer_ui_lib::menu_spec::{NodeSpec, menu_bar};
...
mod menu_bar_helper {
    use super::*;
    pub type Roots = sourcerer_ui_lib::menu_spec::RootSpec;
