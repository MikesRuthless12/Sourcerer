...
pub fn build_app_menu<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<Menu<R>> {
    ...
    // macOS-required app menu (Sourcerer → About / Preferences / Hide / Quit).
    if cfg!(target_os = "macos") {
        let about = MenuItemBuilder::with_id("help.about", "About Sourcerer").build(app)?;
        let prefs = MenuItemBuilder::with_id("tools.options", "Preferences…")
            .accelerator("CmdOrCtrl+,")
            .build(app)?;
        let quit = MenuItemBuilder::with_id("file.exit", "Quit Sourcerer")
            .accelerator("CmdOrCtrl+Q")
            .build(app)?;
        let sep1 = PredefinedMenuItem::separator(app)?;
        let sep2 = PredefinedMenuItem::separator(app)?;
        let app_menu = SubmenuBuilder::new(app, "Sourcerer")
            .item(&about)
            .item(&sep1)
            .item(&prefs)
            .item(&sep2)
