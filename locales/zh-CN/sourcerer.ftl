...
settings-flists-format-srcb = Sourcerer 捆绑包（.srcb）
...
wizard-locale-hint = Sourcerer 提供 18 种语言。稍后可随时切换。
...
# UI panel.
ui-hint = 主题、托盘 / 菜单栏集成、边输入边搜索、行密度。与 voidtools-Everything 直接对等，并附带标有 (+) 的 Sourcerer 新增功能。
ui-section-theme = 主题
...
# Home panel.
home-hint = 应用启动时加载的默认值——每个下拉菜单都可固定为“使用上次的值”或固定的某个值。透镜可见性 / 结果上限为 Sourcerer 新增功能 (+)。
...
folders-section-extras = Sourcerer 附加功能 (+)
...
menu-edit-copy-as-bundle-ref = 复制为 Sourcerer 捆绑包引用
...
menu-help-website = Sourcerer 网站
...
section-sourcerer-extras = Sourcerer 附加功能 (+)
section-sourcerer-additions = Sourcerer 新增功能 (+)
section-sourcerer-extensions = Sourcerer 扩展功能 (+)
