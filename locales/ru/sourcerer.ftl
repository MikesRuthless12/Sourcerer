...
wizard-roots-hint = Добавьте папки или тома, которые Sourcerer будет отслеживать. Это можно изменить позже в настройках индексов.
wizard-browse = Обзор…
...
wizard-locale-hint = Sourcerer доступен на 18 языках. Язык можно сменить позже.
...
# UI panel.
ui-hint = Тема, интеграция с треем / строкой меню, поиск по мере ввода, плотность строк. Прямой паритет с voidtools-Everything плюс дополнения Sourcerer, отмеченные (+).
...
# Home panel.
home-hint = Значения по умолчанию загружаются при запуске — каждый список может оставаться на «Использовать последнее значение» или закреплять фиксированное. Видимость линз / лимиты результатов — дополнения Sourcerer (+).
...
section-sourcerer-additions = Дополнения Sourcerer (+)
