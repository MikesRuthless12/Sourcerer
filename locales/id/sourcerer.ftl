...
wizard-roots-hint = Tambahkan folder atau volume yang ingin dipantau Sourcerer. Anda dapat mengubahnya nanti dari pengaturan Indeks.
...
wizard-locale-hint = Sourcerer tersedia dalam 18 bahasa. Anda dapat beralih nanti.
...
# UI panel.
ui-hint = Tema, integrasi baki / bilah menu, cari sambil mengetik, kepadatan baris. Kesetaraan langsung dengan voidtools-Everything plus tambahan Sourcerer yang ditandai dengan (+).
...
# Home panel.
home-hint = Bawaan dimuat saat aplikasi dijalankan — setiap menu turun dapat tetap di "Gunakan nilai terakhir" atau menyematkan nilai tetap. Visibilitas lensa / batas hasil adalah tambahan Sourcerer (+).
...
folders-section-extras = Tambahan Sourcerer (+)
...
menu-edit-copy-as-bundle-ref = Salin sebagai referensi Sourcerer Bundle
...
section-sourcerer-additions = Tambahan Sourcerer (+)
