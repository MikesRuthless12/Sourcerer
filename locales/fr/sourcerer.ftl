...
wizard-roots-hint = Ajoutez les dossiers ou volumes que vous souhaitez faire surveiller par Sourcerer. Vous pourrez les modifier ultérieurement dans les paramètres des index.
...
# UI panel.
ui-hint = Thème, intégration à la zone de notification / barre de menus, recherche à la frappe, densité des lignes. Parité directe avec voidtools-Everything, plus les ajouts Sourcerer marqués d'un (+).
...
# Home panel.
home-hint = Valeurs par défaut chargées au lancement de l'application — chaque menu déroulant peut conserver « Utiliser la dernière valeur » ou figer une valeur fixe. La visibilité des filtres / les limites de résultats sont des ajouts Sourcerer (+).
...
folders-section-extras = Extras Sourcerer (+)
...
# File menu.
menu-file-hint = Contient des commandes pour travailler avec Sourcerer.
...
menu-help-help = Aide de Sourcerer
