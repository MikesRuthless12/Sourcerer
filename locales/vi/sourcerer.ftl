...
settings-ui-new-window-on-launch = Mở cửa sổ mới khi khởi chạy Sourcerer
...
settings-context-menu-send-to = Gửi tới Sourcerer (đường dẫn)
...
settings-flists-format-srcb = Gói Sourcerer (.srcb)
...
wizard-roots-hint = Thêm các thư mục hoặc ổ đĩa bạn muốn Sourcerer theo dõi. Bạn có thể thay đổi sau trong cài đặt Chỉ mục.
wizard-browse = Duyệt…
...
wizard-locale-hint = Sourcerer có sẵn 18 ngôn ngữ. Bạn có thể chuyển sau.
...
# UI panel.
ui-hint = Giao diện, tích hợp khay / thanh menu, tìm kiếm khi đang gõ, mật độ hàng. Tương đương trực tiếp với voidtools-Everything cùng các bổ sung của Sourcerer được đánh dấu (+).
...
# Home panel.
home-hint = Mặc định được tải khi khởi chạy ứng dụng — mỗi danh sách thả xuống có thể giữ "Dùng giá trị gần nhất" hoặc ghim một giá trị cố định. Hiển thị ống kính / giới hạn kết quả là các bổ sung của Sourcerer (+).
...
folders-section-extras = Bổ sung của Sourcerer (+)
...
menu-edit-copy-as-bundle-ref = Sao chép dưới dạng tham chiếu Gói Sourcerer
...
menu-help-about = Giới thiệu về Sourcerer…
...
section-sourcerer-extras = Bổ sung của Sourcerer (+)
section-sourcerer-additions = Phần thêm của Sourcerer (+)
section-sourcerer-extensions = Phần mở rộng của Sourcerer (+)
