...
settings-flists-format-srcb = Пакунок Sourcerer (.srcb)
...
wizard-roots-hint = Додайте теки або томи, які Sourcerer має відстежувати. Це можна змінити пізніше в параметрах індексів.
wizard-browse = Огляд…
...
wizard-locale-hint = Sourcerer постачається 18 мовами. Перемкнути можна пізніше.
...
# UI panel.
ui-hint = Тема, інтеграція з лотком / рядком меню, пошук під час набору, щільність рядків. Повна відповідність voidtools-Everything плюс доповнення Sourcerer, позначені (+).
...
# Home panel.
home-hint = Типові значення завантажуються під час запуску застосунку — кожен список може зберігати «Останнє значення» або фіксувати задане. Видимість лінз / обмеження результатів — доповнення Sourcerer (+).
...
folders-section-extras = Додатки Sourcerer (+)
...
menu-edit-copy-as-bundle-ref = Копіювати як посилання на пакунок Sourcerer
...
menu-help-website = Вебсайт Sourcerer
