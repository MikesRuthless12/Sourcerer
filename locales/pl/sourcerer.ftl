...
settings-ui-new-window-on-launch = Otwieraj nowe okno przy uruchamianiu Sourcerer
...
wizard-roots-hint = Dodaj foldery lub woluminy, które ma obserwować Sourcerer. Możesz to później zmienić w ustawieniach Indeksów.
...
wizard-locale-hint = Sourcerer jest dostępny w 18 językach. Możesz przełączyć później.
...
# Home panel.
home-hint = Domyślne wartości wczytywane przy uruchomieniu aplikacji — każda lista rozwijana może pozostać przy „Użyj ostatniej wartości” lub przypiąć stałą wartość. Widoczność soczewek / limity wyników to dodatki Sourcerer (+).
...
menu-help-about = O programie Sourcerer…
...
section-sourcerer-additions = Rozszerzenia Sourcerer (+)
