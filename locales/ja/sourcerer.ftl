...
settings-ui-new-window-on-launch = Sourcerer の起動時に新しいウィンドウを開く
...
settings-context-menu-send-to = Sourcerer に送る（パス）
...
wizard-roots-hint = Sourcerer に監視させるフォルダーやボリュームを追加します。これは後で「インデックス」設定から変更できます。
wizard-browse = 参照…
wizard-roots-placeholder = …またはパスを貼り付け
wizard-roots-add = 追加
wizard-roots-remove = 削除
wizard-roots-empty = ルートはまだ設定されていません。
wizard-locale-hint = Sourcerer は 18 言語に対応しています。後で切り替えられます。
...
# UI panel.
ui-hint = テーマ、トレイ／メニューバー統合、入力に合わせた検索、行の表示密度。voidtools-Everything と直接同等の機能に加え、(+) 印の Sourcerer 独自機能を含みます。
...
# Home panel.
home-hint = アプリ起動時に読み込まれる既定値です。各ドロップダウンは「最後の値を使用」のままにすることも、固定値を指定することもできます。レンズの表示状態／結果上限は Sourcerer 独自の機能です (+)。
...
folders-section-extras = Sourcerer 独自機能 (+)
...
# File menu.
menu-file-hint = Sourcerer を操作するためのコマンドが含まれます。
...
section-sourcerer-extras = Sourcerer 独自機能 (+)
