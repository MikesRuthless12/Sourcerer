...
settings-ui-new-window-on-launch = Sourcerer başlatılırken yeni pencere aç
...
# UI panel.
ui-hint = Tema, tepsi / menü çubuğu tümleştirmesi, yazarken arama, satır yoğunluğu. Doğrudan voidtools-Everything denkliği artı (+) ile işaretlenmiş Sourcerer eklentileri.
...
# Home panel.
home-hint = Uygulama başlangıcında yüklenen varsayılanlar — her açılır menü "Son değeri kullan" seçeneğinde kalabilir veya sabit bir değere sabitlenebilir. Mercek görünürlüğü / sonuç sınırları Sourcerer eklentileridir (+).
...
# File menu.
menu-file-hint = Sourcerer ile çalışmaya yönelik komutları içerir.
...
section-sourcerer-extras = Sourcerer Ekstraları (+)
section-sourcerer-additions = Sourcerer Eklentileri (+)
