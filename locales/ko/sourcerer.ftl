...
wizard-locale-hint = Sourcerer는 18개 언어로 제공됩니다. 나중에 바꿀 수 있습니다.
...
# UI panel.
ui-hint = 테마, 트레이 / 메뉴 막대 통합, 입력하는 대로 검색, 행 밀도. voidtools-Everything와 동일한 기능에 Sourcerer 추가 항목은 (+)로 표시됩니다.
ui-section-theme = 테마
...
# Home panel.
home-hint = 앱 실행 시 불러오는 기본값 — 모든 드롭다운은 "마지막 값 사용"을 유지하거나 고정 값을 지정할 수 있습니다. 렌즈 표시 여부 / 결과 제한은 Sourcerer 추가 항목입니다 (+).
...
# File menu.
menu-file-hint = Sourcerer 작업용 명령이 들어 있습니다.
